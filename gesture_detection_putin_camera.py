...
while camera.isOpened():
    ...
    if isBgCaptured == 1:
        ...
        # Draw the text
        cv2.putText(thresh, "Prediction: {0} ({1}%)".format(prediction,score), (50, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,(255, 255, 255))
        cv2.putText(thresh, "Action: {0}".format(action), (50, 80), cv2.FONT_HERSHEY_SIMPLEX, 1,(255, 255, 255))  # Draw the text
    ...
    if k == 27:  # press ESC to exit all windows at any time
        ...
        if save_images:
            img_name = "./frames/drawings/drawing_{0}_{1}.jpg".format(selected_gesture,img_counter)
            cv2.imwrite(img_name, drawing)
            print("{} written".format(img_name))

            img_name2 = "./frames/silhouettes/{0}_{1}.jpg".format(selected_gesture,img_counter)
            cv2.imwrite(img_name2, thresh)
            print("{} written".format(img_name2))

            img_name3 = "./frames/masks/mask_{0}_{1}.jpg".format(selected_gesture,img_counter)
